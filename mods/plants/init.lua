    ...
    for x = minp.x, maxp.x do
		...
		if minetest.env:get_node({x = x, y = y, z = z}).name == "default:dirt_with_grass" then
			...
			if minetest.env:get_node({x = x, y = y + 1, z = z}).name == "air" then
					...
					minetest.env:add_node({x = x, y = y + 1, z = z}, {name="default:pumpkin"})
					end
			end
		end
	    end
...
minetest.register_on_generated(function(minp, maxp)
    for x = minp.x, maxp.x do
        for z = minp.z, maxp.z do
            for ly = minp.y, maxp.y do
                local y = maxp.y + minp.y - ly
		if minetest.env:get_node({x = x, y = y, z = z}).name == "default:dirth_with_gras" then
			local i = math.random(1,555)
			if minetest.env:get_node({x = x, y = y + 1, z = z}).name == "air" then
					if i == 4 or i == 5 or i == 6 then
					minetest.env:add_node({x = x, y = y + 1, z = z}, {name="default:grass"})
					end
			end
		end
	    end
